...
File.open("pretty_print_accurancy.txt","w") do |f|
  0.upto(TIMES) do
arri = ARRI_AIRPORT.sample
depart = DEPART_AIRPORT.sample
rand_time = time_rand(TIME_RANGE_START, TIME_RANGE_END)
    ...
    params = {
      ...
      arri_airport: arri,
      depart_time: rand_time.strftime('%F'),

      return_airport: arri,
      home_airport: depart,
      back_time: (rand_time + rand(1..15)*86400).strftime('%F'),
